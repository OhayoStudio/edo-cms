...
- **Replaced EdoCms placeholder assets** with a generic edo-cms-branded mark so the template doesn't ship someone else's brand.
...
- **Genericized** `bin/deploy-{production,staging}` — replaced hardcoded EdoCms URLs / `ghcr.io/ohayostudio/edo` image tags / production IPs with env-driven placeholders (`REGISTRY_IMAGE`, `DEPLOY_HOST`, `APPLICATION_HOST`). [`5e2c15d`]
...
- **`.gitignore`** — removed vestigial `edo-cms*.json` patterns.
